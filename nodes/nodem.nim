...
macro nimport*(address: string, fn: typed) =
  ...
  of 0:
    quote do:
      return call_nimported_function(`address`, `full_name`, typeof `rtype`)
  of 1:
    let a = args[0][0]
    quote do:
      return call_nimported_function(`address`, `full_name`, `a`, typeof `rtype`)
  of 2:
    let (a, b) = (args[0][0], args[1][0])
    quote do:
      return call_nimported_function(`address`, `full_name`, `a`, `b`, typeof `rtype`)
  of 3:
    let (a, b, c) = (args[0][0], args[1][0], args[2][0])
    quote do:
      return call_nimported_function(`address`, `full_name`, `a`, `b`, `c`, typeof `rtype`)
  else:
    quote do:
...
proc call_nimported_function(address: string, fname: string, args: JsonNode): JsonNode =
  assert args.kind == JArray
  let res = wait_for Address(address).call((fname: fname, args: args).`%`.`$`)
  let data = res.parse_json
  if data["is_error"].get_bool: throw data["message"].get_str
  data["result"]

proc call_nimported_function*[R](address: string, fname: string, rtype: type[R]): R =
  let args = newJArray()
  call_nimported_function(address, fname, args).to(R)

proc call_nimported_function*[A, R](address: string, fname: string, a: A, tr: type[R]): R =
  let args = newJArray(); args.add %a
  call_nimported_function(address, fname, args).to(R)

proc call_nimported_function*[A, B, R](address: string, fname: string, a: A, b: B, tr: type[R]): R =
  let args = newJArray(); args.add %a; args.add %b;
  call_nimported_function(address, fname, args).to(R)

proc call_nimported_function*[A, B, C, R](address: string, fname: string, a: A, b: B, c: C, tr: type[R]): R =
...
proc generate_nimport*(address: Address, folder: string, prepend = default_prepend): void =
  ...
  for nfn in nexported_functions.values:
    ...
    statements.add fmt"""proc {fsign[0]}*({args_s}): {fsign[2]} = nimport("{address}", {fsign[0]})"""
