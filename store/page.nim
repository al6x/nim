...
with Session:
  proc `$`*(self): string = "Session " & self.id
  proc hash*(self): Hash = self.id.hash
  proc `==`*(self, other: Session): bool = self.id == other.id

var sessions: Table[string, Session]

proc process(session: Session, event: JsonNode): Option[JsonNode] =
  (%{ eval: "console.log(\"ok\")" }).some

proc process_sessions() =
  for _, session in sessions:
    while session.inbox.len > 0:
      let res = session.process session.inbox.pop_first
      if not res.is_empty: session.outbox.add_last(res.get)
...
proc http_handler(req: Request): Future[void] {.async, gcsafe.} =
  ...
  if req.req_method == HttpGet:
    if url.path =~ re"^/_client/":
      ...
      log.with(session_id).info("opened")
    ...
    let session = sessions.mget(session_id, () => Session(id: session_id))

    if etype == "event":
      session.inbox.add_last(data)
      log.with(session_id).info("event")
      await req.respond_json(%{})
    elif etype == "pull":
      var res: JsonNode
      while true:
        if session.outbox.len > 0:
          res = session.outbox.pop_first
          break
        await sleep_async(1)
      await req.respond_json(res)
    else:
...
spawn_async server.serve(Port(5000), http_handler, "localhost")
info "started"
while true:
  poll(1)
  process_sessions()
...
#   proc log(self): Log = Log.init("PubSub").with(self.id)
...
# # impl ---------------------------------------------------------------------------------------------
# proc start_gc(ps: PubSub): asyncfutures.Future[void]
...
#   if ps notin impls: throw fmt"pubsub '{ps}' implementation not defined"
#   impls[ps]
...
#   for session_id in impl.sessions.keys: ps.publish_for_client(session_id, ping_msg)
...
# http_handler -------------------------------------------------------------------------------------
# proc http_handler*(req: asynchttpserver.Request): Future[void] {.async.} =
#   try:
...
#   except Exception as e:
#     try:
#       await asynchttpserver.respond(req, httpcore.Http400, "Error, see logs")
