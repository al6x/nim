...
Also, while it's possible to limit re-render to only specific component or specific list of components, it's not worth it (see the `(update: AppEl)` in Twitter Example). It's better to keep things simple and let machines do the work. Keep things simple and respond with the whole page if possible.
