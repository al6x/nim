import asyncdispatch, strutils, options, uri, times, tables
...
# receive ------------------------------------------------------------------------------------------
const delay_ms = 100
proc receive*(url: string): Future[string] {.async.} =
  ...
  try:
    ...
    if not success: await disconnect(url)
...
# emit ---------------------------------------------------------------------------------------------
proc emit*(url: string, message: string): Future[void] {.async.} =
  # Emit message without reply and don't check if it's delivered or not, never fails
  let (is_error, _, socket) = await connect(url)
  if not is_error:
    try:
      await socket.send_message(message)
    except:
...
# call ---------------------------------------------------------------------------------------------
proc call*(url: string, message: string): Future[string] {.async.} =
...
proc receive*(url: string, handler: AsyncMessageHandler) =
  async_check process(url, handler)
  run_forever()

proc process*(url: string, handler: AsyncMessageHandler): Future[void] {.async.} =
...
proc connect(url: string): Future[tuple[is_error: bool, error: string, socket: AsyncSocket]] {.async.} =
...
# disconnect ---------------------------------------------------------------------------------------
proc disconnect*(url: string): Future[void] {.async.} =
  if url in sockets:
...
if is_main_module:
  ...
  proc client: Future[void] {.async.} =
    echo "client started"
    echo await address.call("process")
    await address.send "quit"
