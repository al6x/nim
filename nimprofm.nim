...
# --------------------------------------------------------------------------------------------------
proc unshift[T](list: var openarray[T], blank: T): T =
  assert list.len > 0
  result = list[0]
...
proc post_process(st: var StackTrace): void =
  while ("/nim-1." in $(st.files[0])) or ("/nim/" in $(st.files[0])):
    discard unshift(st.lines, "")
    discard unshift(st.files, "")
...
proc writeProfile() {.noconv.} =
  ...
  block:
    ...
    echo "total executions of each stack trace:"
    var entries = 0
    ...
    for i in 0..min(100, entries-1):
      if profileData[i].total > 1:
        ...
        echo "Entry: ", i+1, "/", entries, " Calls: ", profileData[i].total // totalCalls, " [sum: ", sum, "; ", sum // totalCalls, "]"
        for ii in 0..high(StackTrace.lines):
          let procname = profileData[i].st[ii]
          let filename = profileData[i].st.files[ii]
          if isNil(procname): break
          echo "  ", $filename & ": " & $procname, " ", perProc[$procname] // totalCalls
    echo "... done"
  # else:
  #   echo "... failed"
